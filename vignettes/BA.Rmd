...
Mt.Wilson.alpha.long <- Mt.Wilson.aplha|>
...
ggplot() +
  ...
  labs(x = 'Months', y = 'Monthly mean transmission [ ]') +
...
part_1 <- Mt.Wilson|>
  ...
  dplyr::mutate(variable = factor(variable,
                levels = c("mean_350", "mean_400", "mean_450", "mean_500", 
                           "mean_600", "mean_700", "mean_800",
                           "mean_1000", "mean_1200", "mean_1600")))|>
  dplyr::filter((month > as_date('1909-01-01') & month < as_date('1911-01-31')))
...
part_2 <- Mt.Wilson|>
  ...
  pivot_longer(cols = starts_with('me'),
               names_to = 'variable',
               values_to = 'values')|>
  dplyr::filter(month > as_date('1914-01-01') & month < as_date('1920-01-01'))

  x <- factor(c("alpha","beta","gamma","alpha","beta"))
levels(x) <- list(A="alpha", B="beta", C="gamma")

ggplot() +
  geom_point(data = part_1, aes(x = month, y = values, color = variable)) +
  stat_smooth(data = part_1, aes(x = month, y = values, color = variable),
              linewidth = 0.5, se = FALSE, na.rm = TRUE, method = 'loess') +
  geom_point(data = part_2, aes(x = month, y = values, color = variable)) +
  stat_smooth(data = part_2, aes(x = month, y = values, color = variable),
              linewidth = 0.5, se = FALSE, na.rm = TRUE, method = 'loess') +
  scale_color_viridis_d(option = "magma",
  guide = guide_legend(override.aes = list(size = 3,
  alpha = 1), title = "Wavelength [nm]")) +  
...
Mt.Wilson_long|>
  ...
  guide = guide_legend(override.aes = list(size = 3,
  alpha = 1), title = "Wavelength [nm]")) +
  scale_fill_discrete(breaks = c("trans_350", "trans_400", "trans_450", 
                                "trans_500", "trans_600", "trans_700", 
                                "trans_800", "trans_900","trans_1200", "trans_1600"), 
                        labels = c(c("350", "400", "450", "500", 
...
```{r message=FALSE, error=FALSE, warning=FALSE}
Mt.Wilson.overview <- Mt.Wilson.alpha|>
  group_by(month = lubridate::floor_date(Date, 'year'))|>
  ...
  y = 0.4, label= "Ksudach [NEEM-S1]", hjust = 0, vjust = -0.2, angle = 90, alpha = 0.5) +  
  annotate("text", x = as.Date("1914-06-04", format = '%Y-%m-%d'),
  y = 0.4, label= "Novarupta [NEEM-S1]", hjust = 0, vjust = -0.2, angle = 90, alpha = 0.5) +  
  annotate("text", x = as.Date("1920-03-19", format = '%Y-%m-%d'),
  y = 0.4, label= "Katla [NEEM-S1]", hjust = 0, vjust = -0.2, angle = 90, alpha = 0.5) + 
  ...
  geom_vline(xintercept = as.numeric(as.Date("1918-10-12")), linetype = 'dotted') + 
  annotate("text", x = as.Date("1912-06-06", format = '%Y-%m-%d'),
  y = 0.4, label= "Novarupta [1912-06-06]", hjust = 0, vjust = -0.2, angle = 90, alpha = 0.5) +
  annotate("text", x = as.Date("1907-03-28", format = '%Y-%m-%d'),
  y = 0.4, label= "Ksudach [1907-03-28]", hjust = 0, vjust = -0.2, angle = 90, alpha = 0.5) +  
  annotate("text", x = as.Date("1918-10-12", format = '%Y-%m-%d'),
  y = 0.4, label= "Katla [1918-10-12]", hjust = 0, vjust = -0.2, angle = 90, alpha = 0.5) +   
...
Mt.Wilson.alpha.overview|>
  ggplot(aes(x = month, y = values, color = variable)) +
  geom_point(aes(y = values, color = variable), alpha = 0.5) +
  stat_smooth(linewidth = 0.5, se = FALSE, na.rm = TRUE, method = 'loess') +
  geom_vline(xintercept = as.numeric(as.Date("1912-06-06")), linetype = 'dotted') +
  annotate("text", x = as.Date("1912-06-06", format = '%Y-%m-%d'),
  y = 0, label= "Katmai [1912-06-06]", hjust = 0, vjust = -0.2, angle = 90) +
...
sm_max = df |> group_by(variable)|> drop_na() |>
  dplyr::mutate(smooth = predict(loess(values~as.numeric(month), 
                                       span=.75, normalize = FALSE, method = 'loess'))) |>
  dplyr::filter(month > as_date('1912-05-01') & month < as_date('1913-01-01')) |>
  dplyr::filter(variable == 'mean_350')
  slice_max(order_by = smooth)

# create the smooth and retain rows with max of smooth, using slice_max
  sm_max = df |>
  drop_na()|>
  dplyr::mutate(smooth = predict(loess(values~as.numeric(month), span=.75)))|>
  dplyr::filter(variable == 'mean_350')|>
  dplyr::filter(smooth == min(smooth))  
...
df %>%
  ...
  geom_point() +
  geom_smooth(span=.5, se=F, na.rm = TRUE) + 
...
pdf('../analysis/monthly_transmission_Mt.Wilson.pdf',
    width = 8, height = 5)
transmission
dev.off()
