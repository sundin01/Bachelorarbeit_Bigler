...
od_annual_mean <- Montezuma|>
  ...
  dplyr::summarise('349' = mean(trans_349, na.rm = TRUE),
                   ...
                   '977' = mean(trans_977, na.rm = TRUE),
                   '1214' = mean(trans_1214, na.rm = TRUE),
                   '1600' = mean(trans_1593, na.rm = TRUE))|>
  pivot_longer(cols = ends_with('0'),
               names_to = 'variable',
               values_to = 'values')|>
  dplyr::mutate(variable = factor(variable,
...
################################################################################
# Overview (1920 - 1930)
Montezuma.1902.1907.trans <- Montezuma_long|>
...
Montezuma.1902.1907.trans

################################################################################
# Zoom in (year 1903)
Montezuma.1903.trans <- Montezuma_long|>
...
ggplot()+
  ...
  # Adapt labels
  annotate("text", x = as.Date("1924-02-15", format = '%Y-%m-%d'),
  y = 0.2, label= "Raikoke [1924-02-15]", hjust = 0, vjust = -0.2, angle = 90) +
...
Montezuma.boxplot <- Montezuma_long|>
    dplyr::filter((Date > as_date('1926-01-01'))) |>
ggplot()+
...
################################################################################
# Zoom in (year 1903)
Montezuma.1903.od <- Montezuma_long|>
  dplyr::mutate_at(c('values'), ~-log(.))|>
...
annual_od <- od_annual_mean|>
  ggplot()+
  annotate("text", x = as.Date("1924-02-15", format = '%Y-%m-%d'),
  y = 1.2, label= "Santa Maria [NEEM-S1]", hjust = 1, vjust = -0.2, angle = 90, alpha = 0.5) + 
  geom_rect(aes(xmin = as.Date("1924-11-24", format = '%Y-%m-%d'), 
                xmax = as.Date("1924-02-15", format = '%Y-%m-%d'), 
                ymin = -Inf, ymax = Inf), 
          color = "lightskyblue1", alpha = 0.01, fill = "lightskyblue1", linewidth = 0.2) + 
  geom_point(aes(x = month, y = values, color = variable))+
  geom_line(aes(x = month, y = values, color = variable), size = 0.1)+  
...
################################################################################
# Overview (1905 - 1920)
Montezuma.alpha.1902.1907 <- Montezuma.alpha.long|>
  dplyr::filter(variable  != 'alpha_900') |>  
  ggplot()+
  annotate("text", x = as.Date("1924-02-15", format = '%Y-%m-%d'),
  y = .2, label= "Santa Maria [NEEM-S1]", hjust = 1, vjust = -0.2, angle = 90, alpha = 0.5) + 
  geom_rect(aes(xmin = as.Date("1924-11-24", format = '%Y-%m-%d'), 
                xmax = as.Date("1924-02-15", format = '%Y-%m-%d'), 
                ymin = -Inf, ymax = Inf), 
          color = "lightskyblue1", alpha = 0.01, fill = "lightskyblue1", linewidth = 0.2) + 
...
Montezuma.alpha.1902.1907

################################################################################
# Zoom in (year 1912)

Montezuma.1903.alpha <- Montezuma.alpha.long|>
  # Change dates in the filter to zoom into another time period
  dplyr::filter(Date > as_date('1903-01-01')) |>
  #dplyr::filter(variable  != 'trans_900') |>
  group_by(variable) |>
...
pdf('../analysis/Montezuma//Transmission/Montezuma.1902.1907.trans.pdf',
    width = 8, height = 5)
Montezuma.1902.1907.trans
dev.off()

pdf('../analysis/Montezuma//Transmission/Montezuma.1903.trans.pdf',
    width = 8, height = 5)
Montezuma.1903.trans
dev.off()
...
pdf('../analysis/Montezuma//Optical_depth/Montezuma.1902.1907.od.pdf',
    width = 8, height = 5)
Montezuma.1902.1907.od
dev.off()

pdf('../analysis/Montezuma//Optical_depth/Montezuma.1903.od .pdf',
    width = 8, height = 5)
Montezuma.1903.od 
dev.off()

pdf('../analysis/Montezuma//Optical_depth/Montezuma.boxplot.alpha.pdf',
    width = 8, height = 5)
Montezuma.boxplot.alpha
dev.off()
...
pdf('../analysis/Montezuma//Angstrom_parameter/Montezuma.alpha.1902.1907.pdf',
    width = 8, height = 5)
Montezuma.alpha.1902.1907
dev.off()

pdf('../analysis/Montezuma//Angstrom_parameter/Montezuma.1903.alpha.pdf',
    width = 8, height = 5)
Montezuma.1903.alpha
dev.off()
