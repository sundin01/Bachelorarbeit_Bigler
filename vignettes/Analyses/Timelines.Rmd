...
mutate <- c("trans_1", "trans_2", "trans_3", "trans_4", 
                           "trans_5", "trans_6", "trans_7", "trans_8",
                           "trans_9", "trans_10", "trans_11")
...
signal.extractor <- function(data1, row_1 = 1, row_2 = 2) {
  ...
  new <- c(new, round(((data1[row_2, i] - data1[row_1, i])/data1[row_1, i]), digits = 3))
  }
  data1[nrow(data1) + 1,] <- new
  data1 <- data1|>slice(n())|>dplyr::rename(c('350 nm' = "trans_1_Mean", '400nm' = "trans_2_Mean", 
...
signal.extractor(Washington.annual|>
    group_by(Date)|>
    summarise_at(vars(mutate), funs(Mean = mean(., na.rm = TRUE)))|>
    dplyr::select(-'Date'), 1, 2) |>
    mean_row(na.rm = TRUE)

# Mt.Wilson --> 1906 to 1908 (Ksudach)
signal.extractor(Mt.Wilson.annual|>
    group_by(Date)|>
    summarise_at(vars(mutate), funs(Mean = mean(., na.rm = TRUE)))|>
...
signal.extractor(Haraqua_Hala.annual|>
    group_by(Date)|>
    summarise_at(vars(mutate), funs(Mean = mean(., na.rm = TRUE)))|>
